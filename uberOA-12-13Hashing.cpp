...
int main()
{
    ...
    while(tc--){
        ...
        vector<int> suffix_b(n);
        
        if(s[0]=='a')prefix_a[0]=1;
        if(s[0]=='b')prefix_b[0]=1;
        if(s[n-1]=='a')suffix_a[n-1]=1;
        if(s[n-1]=='b')suffix_b[n-1]=1;
        ...
        int mini=INT_MAX;
        for(int i=0;i<n-1;i++){
            int m1=min(prefix_a[i],prefix_b[i]);
            int m2=min(suffix_a[i+1],suffix_b[i+1]);
            mini=min(mini,m1+m2);
        }
        cout<<mini<<endl;
    }
